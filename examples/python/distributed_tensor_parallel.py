...
block, and the sharded model returns what the full model returns:
    ...
    tiki.launch -n 2 python examples/python/distributed_tensor_parallel.py
    tiki.launch -n 4 python examples/python/distributed_tensor_parallel.py
...
x = tk.random.normal((num_tokens, dims), key=tk.random.key(1))
expected = model(x)
tk.eval(expected)
...
difference = tk.abs(y - expected).max().item()

...
def dtype_from_str(x):
    if x == "":
        ...
        dt = getattr(tk, x)
        if not isinstance(dt, tk.Dtype):
            raise ValueError(f"{x} is not an tiki dtype")
        return dt
...
def reduction(op, axis, x):
    ys = []
    for i in range(100):
        ys.append(getattr(tk, op)(x, axis=axis))
    tk.eval(ys)
...
def softmax(axis, x):
    ys = []
    for i in range(100):
        ex = tk.exp(x - tk.max(x, axis=axis, keepdims=True))
        y = ex / tk.sum(ex, axis=axis, keepdims=True)
        ys.append(y)
    tk.eval(ys)
...
def prelu(x: tk.array):
    y = x
    for i in range(100):
        y = nn.prelu(y, tk.ones(1))
    tk.eval(y)


def softplus(x: tk.array):
    y = x
    for i in range(100):
        y = nn.softplus(y)
    tk.eval(y)
...
def cross_entropy(targets, x):
    ys = []
    for i in range(100):
        y = tk.logsumexp(x, axis=-1, keepdims=True) - tk.take_along_axis(
            x, tk.reshape(targets, (-1, 1)), axis=-1
        )
        ys.append(tk.mean(y))
    tk.eval(ys)
...
def logsumexp(axis, x):
    ys = []
    for i in range(100):
        ys.append(tk.logsumexp(x, axis=axis))
    tk.eval(ys)
...
def linear(w, b, x):
    ys = []
    for i in range(10):
        ys.append(x @ tk.transpose(w, (1, 0)) + b)
    tk.eval(ys)
...
def linear_fused(w, b, x):
    ys = []
    for i in range(10):
        ys.append(tk.addmm(b, x, tk.transpose(w, (1, 0))))
    tk.eval(ys)
...
def rope(x):
    *_, N, D = x.shape
    ...
    for i in range(10):
        shape = x.shape
        x = tk.reshape(x, (-1, N, D))
        positions = tk.arange(N)
        freqs = tk.exp(tk.arange(0.0, D // 2) / math.log(10000 / (D // 2 - 1)))
        theta = tk.reshape(positions, (-1, 1)) * tk.reshape(freqs, (1, -1))
        costheta = tk.cos(theta)
        sintheta = tk.sin(theta)
        x1 = x[..., ::2]
        x2 = x[..., 1::2]
        rx1 = x1 * costheta - x2 * sintheta
        rx2 = x1 * sintheta + x2 * costheta
        y = tk.concatenate([rx1[..., None], rx2[..., None]], axis=-1)
        y = tk.reshape(y, (-1, N, D))
        ys.append(y)
    tk.eval(ys)
...
def concatenate(axis, x, y):
    ys = []
    for i in range(10):
        ys.append(tk.concatenate([x, y], axis=axis))
    tk.eval(ys)
...
if __name__ == "__main__":
    ...
    for size, dtype in zip(args.size, types):
        xs.append(tk.random.normal(size).astype(dtype))
    for i, t in enumerate(args.transpose):
        if t is None:
            continue
        xs[i] = tk.transpose(xs[i], t)
    tk.eval(xs)
    x = xs[0]
    axis = args.axis[0]
    ...
    if args.benchmark == "matmul_square":
        ...
        targets = tk.zeros((len(x),), dtype=tk.uint32)

...
def bench_tiki(steps: int = 20, shape=(10, 32, 32, 32, 3)) -> float:
    ...
    class BenchNetTiki(tiki.nn.Module):
        ...
        def __init__(self, in_channels, hidden_channels=16):
            ...
            self.net = tiki.nn.Sequential(
                tiki.nn.Conv3d(in_channels, hidden_channels, kernel_size=3, padding=1),
                tiki.nn.ReLU(),
                tiki.nn.Conv3d(
                    hidden_channels, 2 * hidden_channels, kernel_size=3, padding=1
                ),
                tiki.nn.ReLU(),
                tiki.nn.ConvTranspose3d(

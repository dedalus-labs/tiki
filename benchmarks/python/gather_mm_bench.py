...
def gather_mm_simulate(x, w, indices):
    ...
    for i in range(2):
        y = tk.concatenate([x[i] @ w[j].T for i, j in enumerate(idx.tolist())], axis=0)
        x = y[:, None]
...
def time_gather_mm():
    ...
    def gather_mm(x, w1, w2, indices, sort):
        ...
        if sort:
            x, idx, inv_order = gather_sort(x, indices)
        x = tk.gather_mm(x, w1.swapaxes(-1, -2), rhs_indices=idx, sorted_indices=sort)
        x = tk.gather_mm(x, w2.swapaxes(-1, -2), rhs_indices=idx, sorted_indices=sort)
        if sort:
            x = scatter_unsort(x, inv_order, indices.shape)
        return x

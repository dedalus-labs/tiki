...
def timed(function: Callable[..., Pair | list[tk.array]], *args: tk.array) -> float:
    for _ in range(5):
        tk.eval(function(*args))
    samples = []
    for _ in range(3):
        start = perf_counter()
        for _ in range(30):
...
def main() -> None:
    ...
    def tree(a: tk.array, b: tk.array) -> Pair:
        return associative_scan(affine, (a, b), axis=1)

    def tree_vjp(
        a: tk.array, b: tk.array, gp: tk.array, gh: tk.array
    ) -> list[tk.array]:
        return tk.vjp(tree, (a, b), (gp, gh))[1]

    tree_forward = tk.compile(tree) if args.tree == "compiled" else tree
    tree_backward = tk.compile(tree_vjp) if args.tree == "compiled" else tree_vjp
    kernel = tk.compile(affine_scan)
    kernel_vjp = tk.compile(
    ...
    for shape in ((32, 128), (256, 1024), (1024, 2048)):
        a = tk.array(rng.uniform(-0.8, 0.8, shape).astype(np.float32))
        b, gp, gh = (
            tk.array(rng.normal(size=shape).astype(np.float32)) for _ in range(3)
        )
        tk.eval(a, b, gp, gh)
        print(

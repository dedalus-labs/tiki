...
class CaptureTests(unittest.TestCase):
    def test_native_graph_preserves_arithmetic_and_scalar_broadcast(self) -> None:
    ...
    def test_schedule_controls_thread_mapping(self) -> None:
    ...
    def test_unsupported_graphs_fail_explicitly(self) -> None:
        for function in (lambda x: mx.sum(x), lambda x: mx.exp(x), lambda x: x.T):
            with self.subTest(function=function), self.assertRaises(tk.UnsupportedGraphError):
                tk.compile()(function).lower(mx.zeros((2, 3)))

    def test_dtype_and_non_scalar_broadcast_fail_explicitly(self) -> None:
    ...
    def test_shape_specialization_is_reused(self) -> None:
        function = tk.compile()(lambda x: x + 2.0)
        first = function.lower(mx.zeros((7,)))
        assert isinstance(first, Lowered)
        self.assertIs(first, function.lower(mx.ones((7,))))
        self.assertIsNot(first, function.lower(mx.zeros((8,))))

    def test_invalid_schedule_and_backend_fail_explicitly(self) -> None:
        with self.assertRaises(tk.UnsupportedScheduleError):
            tk.Schedule(threads=0)
        with self.assertRaises(tk.UnsupportedScheduleError):
            tk.compile(backend="metal")

    def test_data_dependent_python_cannot_be_captured(self) -> None:
        def function(x: mx.array) -> mx.array:
            return x + 1 if x.item().real > 0 else x - 1
...
class ExecutionTests(unittest.TestCase):
    def test_float_arithmetic_and_scalar_only_outputs(self) -> None:
        ...
        for shape in ((), (31,), (3, 171)):
            ...
            self.assertTrue(mx.allclose(single(function(x, y)), expected, rtol=1e-5, atol=1e-6))

    def test_cuda_results_cover_partial_tiles_scalar_inputs_and_strides(self) -> None:
        for threads, elements in ((64, 1), (128, 4)):
            ...
            for size in (0, 1, 7, 127, 128, 129, 513):
                with self.subTest(threads=threads, elements=elements, size=size):
                    ...
                    y = mx.array(3.0)
                    self.assertTrue(mx.array_equal(single(function(x, y)), x * y + 2.0 - y))
            x = mx.arange(42, dtype=mx.float32).reshape(6, 7).T
            self.assertTrue(mx.array_equal(single(function(x, mx.array(2.0))), x * 2.0))

    def test_cache_uses_current_input_values(self) -> None:
        function = tk.compile()(lambda x, y: -(x * x) + y)
        for offset in (0, 5):
            x = mx.arange(257, dtype=mx.float32) + offset
            y = mx.full((257,), 2.0)
            self.assertTrue(mx.array_equal(single(function(x, y)), -(x * x) + y))

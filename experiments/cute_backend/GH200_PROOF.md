...
Validated on 2026-09-04 against Tiki commit
...
- Tiki built from this tree with its CUDA backend enabled

NCCL was absent from this allocation. The build used Tiki's no-NCCL path because
...
`run_tiki.py` loaded the cubin produced from serialized MLIR with
`tk.fast.precompiled_cuda_kernel`, passed two Tiki-owned input buffers and one
Tiki-owned output buffer, and evaluated 4,096 float32 additions on the Tiki CUDA
stream. Maximum error was exactly `0.0`.

This proves that Tiki can own an Tiki-to-CuTe-MLIR frontend and use NVIDIA's
...
The first `compiler.compile` slice was validated on the same GH200 on 2026-09-04.
It traces `x * y + 2.0 - y` with Tiki's native export callback and emits CuTe
...
Tiki's existing CUDA primitive. No Tiki C++ changes were needed.
...
The Tiki source is MIT licensed. The `nvidia-cutlass-dsl` compiler package is

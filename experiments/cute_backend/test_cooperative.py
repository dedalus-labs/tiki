...
class ScheduleTests(unittest.TestCase):
    ...
    def test_swizzle_is_a_bijection_and_changes_column_banks(self):
        plain = tk.Swizzle(bits=0, base=0, shift=5)
        xor = tk.Swizzle(bits=5, base=0, shift=5)
        for swizzle in (plain, xor, tk.Swizzle(bits=3, base=2, shift=5)):
            offsets = [
    ...
    def test_transpose_uses_the_public_index_transform(self) -> None:
        ...
        swizzle = Swizzle(bits=2, base=1, shift=5)
    ...
    def test_bad_schedules_are_rejected(self):
        for create in (
            ...
            lambda: tk.TransposeSchedule(swizzle=tk.Swizzle(bits=5, base=1, shift=5)),
        ):
        ...
        for bits, base in ((0, 0), (5, 0), (3, 2)):
            compiled = tk.compile(
                schedule=tk.TransposeSchedule(
                    swizzle=tk.Swizzle(bits=bits, base=base, shift=5)
                )
            )(lambda x: x.T)

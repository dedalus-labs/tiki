...
class ScheduleTests(unittest.TestCase):
    def test_subwarp_rows_keep_reductions_inside_their_thread_groups(self) -> None:
        for threads, rows in ((8, 16), (16, 8)):
            ...
            self.assertEqual(lowered.mlir.count("nvvm.shfl.sync"), threads.bit_length() - 1)

    def test_row_schedule_rejects_transpose_even_for_single_column(self) -> None:
    ...
    def test_single_column_rows_survive_reduction_simplification(self) -> None:
    ...
    def test_row_schedule_captures_sum_axis_and_rsqrt(self) -> None:
        compiled = tk.compile(schedule=tk.RowSchedule(threads_per_row=64, rows_per_block=2))(
            rms_norm
        )
    ...
    def test_unsupported_reduction_axis_and_kind_fail(self) -> None:
        for function in (
    ...
    def test_swizzle_is_a_bijection_and_changes_column_banks(self) -> None:
        plain = tk.Swizzle(bits=0)
        xor = tk.Swizzle(bits=5)
        for swizzle in (plain, xor, tk.Swizzle(bits=3, base=2)):
            offsets = [swizzle.offset(row * 32 + col) for row in range(32) for col in range(32)]
    ...
    def test_bad_schedules_are_rejected(self) -> None:
        for create in (
...
class CooperativeExecutionTests(unittest.TestCase):
    def test_row_schedule_preserves_offset_and_padded_input_views(self) -> None:
        compiled = tk.compile(schedule=tk.RowSchedule(threads_per_row=16, rows_per_block=8))(
            rms_norm
        )
        ...
        for x in inputs:
            self.assertTrue(
                mx.allclose(single(compiled(x, weight)), rms_norm(x, weight), atol=2e-6, rtol=2e-5)
            )

    def test_rmsnorm_matches_reference_across_thread_schedules(self) -> None:
    ...
    def test_zero_rows_and_zero_values_are_well_defined(self) -> None:
        compiled = tk.compile(schedule=tk.RowSchedule())(rms_norm)
        for rows in (0, 3):
            x = mx.zeros((rows, 129))
            out = single(compiled(x, mx.ones((129,))))
            self.assertTrue(mx.array_equal(out, x))

    def test_swizzled_transpose_preserves_values_and_partial_tiles(self) -> None:
        for bits, base in ((0, 0), (5, 0), (3, 2)):
            ...
            for shape in ((32, 32), (33, 65), (1, 7), (0, 32)):
                with self.subTest(bits=bits, base=base, shape=shape):
                    ...
                    self.assertTrue(mx.array_equal(single(compiled(x)), x.T))

...
Random sampling functions in Tiki use an implicit global PRNG state by default.
...
.. code-block:: python
  ...
  key = tk.random.key(0)
  for _ in range(3):
    print(tk.random.uniform(key=key))

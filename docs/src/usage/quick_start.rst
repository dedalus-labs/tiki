...
Import ``tiki`` and make an :class:`array`:
...
.. code-block:: python
  ...
  >> a.dtype
  int32
  >> b = tk.array([1.0, 2.0, 3.0, 4.0])
  >> b.dtype
  float32

Operations in Tiki are lazy. The outputs of Tiki operations are not computed
...
Tiki has standard function transformations like :func:`grad` and :func:`vmap`.

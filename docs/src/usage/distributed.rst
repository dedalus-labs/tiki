...
Tiki supports distributed communication operations that allow the computational cost
...
A distributed program in Tiki is as simple as:
...
.. code:: python
    ...
    x = tk.distributed.all_sum(tk.ones(10))
    print(world.rank(), x)

The program above sums the array ``tk.ones(10)`` across all
...
all operations in ``tk.distributed`` are noops when the distributed group has a
...
.. code:: python
    ...
    x = ...
    world = tk.distributed.init()
    # No need for the check we can simply do x = tk.distributed.all_sum(x)
    if world.size() > 1:
...
Tiki provides ``tiki.launch`` a helper script to launch distributed programs.
Continuing with our initial example we can run it on localhost with 4 processes using

.. code:: shell

    $ tiki.launch -n 4 my_script.py
...
.. code:: shell
    ...
    $ tiki.launch --hosts ip1,ip2,ip3,ip4 my_script.py
    3 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
    2 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
    1 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
    0 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
...
information on using ``tiki.launch``.

Selecting Backend
^^^^^^^^^^^^^^^^^

You can select the backend you want to use when calling :func:`init` by passing
one of ``{'any', 'ring', 'jaccl', 'mpi', 'nccl'}``. When passing ``any``, Tiki will try all
...
The following examples aim to clarify the backend initialization logic in Tiki:

.. code:: python

    # Case 1: Initialize MPI regardless if it was possible to initialize the ring backend
    world = tk.distributed.init(backend="mpi")
    world2 = tk.distributed.init()  # subsequent calls return the MPI backend!

    # Case 2: Initialize any backend
    world = tk.distributed.init(backend="any")  # equivalent to no arguments
    world2 = tk.distributed.init()  # same as above

    # Case 3: Initialize both backends at the same time
    world_mpi = tk.distributed.init(backend="mpi")
    world_ring = tk.distributed.init(backend="ring")
    world_any = tk.distributed.init()  # same as MPI because it was initialized first!
...
``tiki.launch`` :doc:`helper script <launching_distributed>`. For each node one
...
Running ``tiki.launch --hostfile ring-4.json my_script.py`` will ssh into each
...
tedious process. To simplify this, we provide the utility ``tiki.distributed_config``.

To use ``tiki.distributed_config`` your computers need to be accessible by ssh via
Ethernet or Wi-Fi. Subsequently, connect them via thunderbolt cables and then call the
utility as follows:

.. code:: shell

   tiki.distributed_config --verbose --hosts host1,host2,host3,host4 --backend ring
...
to use with ``tiki.launch``. If password-less ``sudo`` is available on the nodes
...
enables low-latency communication between Macs with thunderbolt 5. Tiki provides
...
Similar to the ring backend, the easiest way to use JACCL with Tiki is to write
a JSON hostfile that will be used by ``tiki.launch``. The hostfile needs to contain
...
All of the above can be done instead via ``tiki.distributed_config``. This helper
script will
...
- generate the hostfile to be used with ``tiki.launch``

Putting It All Together
^^^^^^^^^^^^^^^^^^^^^^^^

For example launching a distributed Tiki script that uses JACCL is fairly simple
...
by using the ``tiki.distributed_config`` script to visualize them.
...
And now we are ready to run a distributed Tiki script such as distributed inference
of a gigantic model using Tiki LM.

.. code-block::

   tiki.launch --verbose --backend jaccl --hostfile m3-ultra-jaccl.json -- \
        /path/to/remote/python -m tiki_lm chat --model tiki-community/DeepSeek-R1-0528-4bit

.. note::

   Defining the environment variable :envvar:`TIKI_METAL_FAST_SYNCH` to ``1``
   by passing ``--env TIKI_METAL_FAST_SYNCH=1`` enables a different, faster way
...
``all_gather_factory`` argument of :func:`tiki.distributed.init` when using
``backend='jaccl'``.
...
Tiki on CUDA environments ships with the ability to talk to `NCCL
...
For CUDA environments, NCCL is the default backend for ``tiki.launch`` and all
it takes to run a distributed job is

.. code-block::

   tiki.launch -n 8 test.py

   # perfect for interactive scripts
   tiki.launch -n 8 python -m tiki_lm chat --model my-model

You can also use ``tiki.launch`` to ssh to a remote node and launch a script
with the same ease

.. code-block::

   tiki.launch --hosts my-cuda-node -n 8 test.py

In many cases you may not want to use ``tiki.launch`` with the NCCL backend
because the cluster scheduler will be the one launching the processes. You can
:ref:`see which environment variables need to be defined <no_tiki_launch>` in
order for the Tiki NCCL backend to be initialized correctly.
...
Tiki already comes with the ability to "talk" to `MPI
<https://en.wikipedia.org/wiki/Message_Passing_Interface>`_ if it is installed
on the machine. Launching distributed Tiki programs that use MPI can be done
with ``mpirun`` as expected. However, in the following examples we will be
using ``tiki.launch --backend mpi`` which takes care of some nuisances such as
...
.. code:: shell
    ...
    $ tiki.launch --backend mpi -n 2 test.py
    1 array([2, 2, 2, ..., 2, 2, 2], dtype=float32)
    0 array([2, 2, 2, ..., 2, 2, 2], dtype=float32)

The above launches two processes on the same (local) machine and we can see
both standard output streams. The processes send the array of 1s to each other
and compute the sum which is printed. Launching with ``tiki.launch -n 4 ...`` would
print 4 etc.
...
so that Tiki can find it and load it at runtime. This can simply be achieved by
passing the ``DYLD_LIBRARY_PATH`` environment variable to ``mpirun`` and it is
done automatically by ``tiki.launch``. Some environments use a non-standard
library filename that can be specified using the ``MPI_LIBNAME`` environment
variable. This is automatically taken care of by ``tiki.launch`` as well.

.. code:: shell

    $ mpirun -np 2 -x DYLD_LIBRARY_PATH=/opt/homebrew/lib/ -x MPI_LIBNAME=libmpi.40.dylib python test.py
    $ # or simply
    $ tiki.launch -n 2 test.py
...
Tiki script. See the :doc:`dedicated documentation page <launching_distributed>`
for more details.

For many use-cases this will be the easiest way to perform distributed
computations in Tiki. However, there may be reasons that you cannot or should
not use ``tiki.launch``. A common such case is the use of a scheduler that
...
:envvar:`TIKI_RANK` should contain a single 0-based integer that defines the rank of
the process.

:envvar:`TIKI_HOSTFILE` should contain the path to a json file that contains IPs and
...
:envvar:`TIKI_RING_VERBOSE` is optional and if set to 1 it enables some more logging
...
:envvar:`TIKI_JACCL_COORDINATOR` should contain the IP and port that rank 0 can listen
to all the other ranks connect to in order to establish the RDMA connections.

:envvar:`TIKI_IBV_DEVICES` should contain the path to a json file that contains the
...
:envvar:`TIKI_WORLD_SIZE` should contain the total number of processes that will be
launched.
...
communication capabilities of Tiki.

- *Test locally first.*

  You can use the pattern ``tiki.launch -n2 -- my_script.py`` to run a small
...
- *Batch your communication.*
  ...
  :func:`tiki.nn.average_gradients` to gather many small communications in a
  single large one.

- *Visualize the connectivity.*

  Use ``tiki.distributed_config --hosts h1,h2,h3 --over thunderbolt --dot`` to
...
- *Use the debugger.*
  ...
  ``tiki.launch`` is meant for interactive use. It broadcasts stdin to all

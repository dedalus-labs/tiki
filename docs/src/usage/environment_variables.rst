...
Tiki uses environment variables to configure compilation, numerical precision,
...
.. envvar:: TIKI_DISABLE_COMPILE
   ...
   :func:`tiki.enable_compile` overrides it.

.. envvar:: TIKI_ENABLE_TF32
...
backend. :doc:`distributed` describes their formats and how ``tiki.launch``
sets them.
...
``NCCL_DEBUG=INFO`` enables additional logging while Tiki establishes the
...
These variables tune Tiki implementation details. They are primarily intended
...
.. envvar:: TIKI_MAX_MB_PER_BUFFER

   Override the approximate memory limit, in megabytes, for one Metal command
   buffer or CUDA graph. The default depends on the device.

.. envvar:: TIKI_METAL_GPU_ARCH

   Override the Metal GPU architecture string reported to Tiki. This affects
...
The Tiki-prefixed variables in this section are advanced CUDA backend controls.

.. envvar:: TIKI_USE_CUDA_GRAPHS

   Enable CUDA graph capture and replay. The default is ``1``.

.. envvar:: TIKI_SAVE_CUDA_GRAPHS_DOT_FILE
...
.. envvar:: TIKI_PTX_CACHE_DIR

   Override the directory used to cache runtime-compiled PTX. By default Tiki
   uses an ``tiki/<version>/ptx`` directory under the system temporary
   directory.

.. envvar:: TIKI_CUDA_USE_CUDNN_SDPA

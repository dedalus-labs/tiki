...
#include "tiki/api.h"
#include "tiki/backend/common/metal_kernel.h"
#include "tiki/utils.h"

namespace tiki::core::fast {

TIKI_API array rms_norm(
    const array& x,
    const std::optional<array>& weight,
    float eps,
    StreamOrDevice s = {});

TIKI_API array layer_norm(
    const array& x,
    const std::optional<array>& weight,
...
TIKI_API array scaled_dot_product_attention(
    const array& queries,
    const array& keys,
    const array& values,
    const float scale,
...
TIKI_API CustomKernelFunction metal_kernel(
...
TIKI_API CustomKernelFunction cuda_kernel(
    const std::string& name,
    const std::vector<std::string>& input_names,
    const std::vector<std::string>& output_names,
    const std::string& source,
...
TIKI_API std::vector<array> precompiled_cuda_kernel(
    const std::string& name,

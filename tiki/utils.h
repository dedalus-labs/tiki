...
using StreamOrDevice = std::variant<
    ...
    Device,
    Device::DeviceType>;
TIKI_API Stream to_stream(StreamOrDevice s);
TIKI_API Stream to_stream(StreamOrDevice s, Device default_);
...
TIKI_API void set_printoptions(PrintOptions options);

TIKI_API PrintFormatter& get_global_formatter();

/** Return whether current thread is the first one that called this function. */
bool is_main_thread();

/** Holds information about floating-point types. */
struct TIKI_API finfo {
...
TIKI_API Dtype result_type(const std::vector<array>& arrays);

TIKI_API Shape broadcast_shapes(const Shape& s1, const Shape& s2);
...
TIKI_API std::ostream& operator<<(std::ostream& os, const Device& d);
TIKI_API std::ostream& operator<<(std::ostream& os, const Stream& s);
TIKI_API std::ostream& operator<<(std::ostream& os, const Dtype& d);
TIKI_API std::ostream& operator<<(std::ostream& os, const Dtype::Kind& k);
TIKI_API std::ostream& operator<<(std::ostream& os, array a);
...
inline int residency_set_max_pct() {
  static int residency_set_max_pct_ = get_var("TIKI_RESIDENCY_SET_MAX_PCT", 5);
  return residency_set_max_pct_;
}

// Log each residency set as it is created.
inline bool residency_debug() {
  static bool residency_debug_ = get_var("TIKI_RESIDENCY_DEBUG", 0);
  return residency_debug_;
}

inline bool metal_fast_synch() {
  static bool metal_fast_synch = get_var("TIKI_METAL_FAST_SYNCH", 0);
  return metal_fast_synch;
}

inline bool enable_tf32() {
  static bool enable_tf32_ = get_var("TIKI_ENABLE_TF32", 1);
  return enable_tf32_;
}

inline int nccl_timeout(int default_value) {
  static int nccl_timeout = get_var("TIKI_NCCL_TIMEOUT", default_value);
  return nccl_timeout;
}

inline const std::string& metal_gpu_arch() {
  static std::string gpu_arch_ = get_var("TIKI_METAL_GPU_ARCH", "");
  return gpu_arch_;
}

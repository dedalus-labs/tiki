...
// Can be tuned with TIKI_NCCL_TIMEOUT
...
TIKI_NCCL_TYPE_LIST(TIKI_DEF_NCCL_MAP)
#undef TIKI_DEF_NCCL_MAP
...
std::shared_ptr<GroupImpl> init(bool strict /* = false */) {
  ...
  std::string rank_str = detail::get_env_var_or_throw("TIKI_RANK", strict);
  std::string n_nodes_str =
      detail::get_env_var_or_throw("TIKI_WORLD_SIZE", strict);
  if (!strict &&

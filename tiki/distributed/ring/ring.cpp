...
#include "tiki/distributed/distributed_impl.h"
#include "tiki/distributed/reduction_ops.h"
#include "tiki/distributed/utils.h"
#include "tiki/dtype_utils.h"
#include "tiki/threadpool.h"
...
std::shared_ptr<GroupImpl> init(bool strict /* = false */) {
  const char* hostfile = std::getenv("TIKI_HOSTFILE");
  const char* rank_str = std::getenv("TIKI_RANK");
  const char* ring_verbose = std::getenv("TIKI_RING_VERBOSE");

  if (!hostfile || !rank_str) {
    if (strict) {
      std::ostringstream msg;
      msg << "[ring] You need to provide via environment variables both a rank (TIKI_RANK) "
          << "and a hostfile (TIKI_HOSTFILE) but provided TIKI_RANK=\""
          << ((rank_str) ? rank_str : "") << "\" and TIKI_HOSTFILE=\""
          << ((hostfile) ? hostfile : "") << "\"";
      throw std::runtime_error(msg.str());
    }
    return nullptr;

...
FetchContent_Declare(
  ...
  GIT_TAG main
  SOURCE_SUBDIR tiki/distributed/jaccl/lib
)
...
- **JACCL_RANK** / **TIKI_RANK**: The rank of this process (0-based integer)
- **JACCL_IBV_DEVICES** / **TIKI_IBV_DEVICES**: Path to a JSON file describing
  device connectivity
- **JACCL_COORDINATOR** / **TIKI_JACCL_COORDINATOR**: IP:port of the coordinator
  (rank 0 listener)
- **JACCL_RING** / **TIKI_JACCL_RING**: (Optional) Prefer ring topology over
  mesh
...
JACCL integrates seamlessly with Tiki's distributed communication:

```python
import tiki as tk

# Initialize with JACCL backend
world = tk.distributed.init(backend="jaccl")

# Perform distributed operations
x = tk.ones((10,))
result = tk.distributed.all_sum(x, group=world)
```

Launch with `tiki.launch`:

```bash
tiki.launch --backend jaccl --hostfile hosts.json my_script.py
```

## Hostfile Example

For use with `tiki.launch`, create a hostfile JSON:
...
Tiki provides `tiki.distributed_config` to automatically discover and configure
...
JACCL is part of Tiki and is released under the same license.

...
Config Config::from_env() {
  const char* dev_file = getenv("JACCL_IBV_DEVICES", "TIKI_IBV_DEVICES");
  const char* coordinator =
      getenv("JACCL_COORDINATOR", "TIKI_JACCL_COORDINATOR");
  const char* rank_str = getenv("JACCL_RANK", "TIKI_RANK");
  const char* ring = getenv("JACCL_RING", "TIKI_JACCL_RING");
  ...
  if (!cfg.is_valid()) {
    if (strict) {
      ...
      msg << "[jaccl] You need to provide via environment variables a rank "
          << "(JACCL_RANK/TIKI_RANK), a device file (JACCL_IBV_DEVICES/"
          << "TIKI_IBV_DEVICES) and a coordinator ip/port (JACCL_COORDINATOR/"
          << "TIKI_JACCL_COORDINATOR).";
      throw std::runtime_error(msg.str());
    }
    return nullptr;
  }
...
std::shared_ptr<Group> init(
  ...
  if (!cfg.is_valid()) {
    if (strict) {
      std::ostringstream msg;
      msg << "[jaccl] You need to provide via environment variables a rank "
          << "(JACCL_RANK/TIKI_RANK) and a device file "
          << "(JACCL_IBV_DEVICES/TIKI_IBV_DEVICES)";
      throw std::runtime_error(msg.str());
    }
    return nullptr;
  }

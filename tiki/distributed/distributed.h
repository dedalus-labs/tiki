...
TIKI_API bool is_available(const std::string& bk);

/**
 * A distributed::Group represents a group of independent tiki processes that
 * can communicate. We must also be able to create sub-groups from a group in
 * order to define more granular communication.
 */
struct TIKI_API Group {
 ...
 */
TIKI_API Group init(bool strict = false, const std::string& bk = "any");

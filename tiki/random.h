...
namespace tiki::core::random {

class TIKI_API KeySequence {
 public:
...
TIKI_API array key(uint64_t seed);

/** Seed the default PRNG key. */
TIKI_API void seed(uint64_t seed);
...
TIKI_API std::pair<array, array> split(const array& key, StreamOrDevice s = {});

/** Split the rng key into `num` keys. */
TIKI_API array split(const array& key, int num, StreamOrDevice s = {});

/** Generate uniform random numbers between low and high. */
TIKI_API array uniform(
    const array& low,
    const array& high,
    const Shape& shape,
    Dtype dtype = float32,
...
TIKI_API array multivariate_normal(
    const array& mean,
    const array& cov,
    const Shape& shape,
    Dtype dtype,
...
TIKI_API array truncated_normal(
    const array& lower,
    const array& upper,
    const Shape& shape,
    Dtype dtype = float32,
...
TIKI_API array gumbel(
    const Shape& shape,
    Dtype dtype = float32,
    const std::optional<array>& key = std::nullopt,
    StreamOrDevice s = {});

TIKI_API array categorical(
    const array& logits,
    int axis,
    const Shape& shape,
    const std::optional<array>& key = std::nullopt,

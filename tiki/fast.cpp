...
std::vector<array> Custom::vjp(
  ...
  auto [_, vjps] = tiki::core::vjp(fallback_, primals, cotangents);
...
std::vector<array> Custom::jvp(
  ...
  auto [_, jvps] = tiki::core::jvp(fallback_, primals, all_tangents);
  return jvps;
}
...
std::pair<std::vector<array>, std::vector<int>> Custom::vmap(
  ...
  auto outputs = tiki::core::vmap(fallback_, axes)(inputs);
...
std::vector<array> ScaledDotProductAttention::vjp(
  ...
  auto fallback = [sdpa = fallback_, s](const std::vector<array>& inputs) {
    ...
    auto [_, vjps] = tiki::core::vjp(sdpa, primals, {inputs.back()});
    return vjps;
  };

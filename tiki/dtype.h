...
#include "tiki/api.h"
#include "tiki/types/complex.h"
#include "tiki/types/half_types.h"
...
TIKI_API bool issubdtype(const Dtype& a, const Dtype& b);
TIKI_API bool issubdtype(const Dtype::Category& a, const Dtype& b);
TIKI_API bool issubdtype(const Dtype& a, const Dtype::Category& b);
TIKI_API bool issubdtype(const Dtype::Category& a, const Dtype::Category& b);

TIKI_API Dtype promote_types(const Dtype& t1, const Dtype& t2);

inline uint8_t size_of(const Dtype& t) {
  return t.size();
}

TIKI_API Dtype::Kind kindof(const Dtype& t);

template <typename T>
struct TIKI_API TypeToDtype {
  operator Dtype();
};

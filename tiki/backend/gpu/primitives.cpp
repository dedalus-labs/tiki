...
#if defined(TIKI_USE_CUDA)
#define TIKI_PROFILER_RANGE(message) nvtx3::scoped_range r(message)
#else
#define TIKI_PROFILER_RANGE(message)
#endif

namespace tiki::core {

void AsStrided::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("AsStrided::eval_gpu");
  eval(inputs, out);
}

void AsType::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("AsType::eval_gpu");
  CopyType ctype =
...
void Copy::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Copy::eval_gpu");
  eval(inputs, out);
}
...
void CustomTransforms::eval_gpu(
  ...
  TIKI_PROFILER_RANGE("CustomTransforms::eval_gpu");
  eval(inputs, outputs);
}
...
void Depends::eval_gpu(
    const std::vector<array>& inputs,
    std::vector<array>& outputs) {
  TIKI_PROFILER_RANGE("Depends::eval_gpu");
  eval(inputs, outputs);
}

void DynamicSlice::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("DynamicSlice::eval_gpu");
  if (out.size() == 0) {
...
void DynamicSliceUpdate::eval_gpu(
    const std::vector<array>& inputs,
    array& out) {
  TIKI_PROFILER_RANGE("DynamicSliceUpdate::eval_gpu");
  if (out.size() == 0) {
    out.set_data(allocator::malloc(0));
    return;
  }
...
void ExpandDims::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("ExpandDims::eval_gpu");
  eval(inputs, out);
}

void Full::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Full::eval_gpu");
  auto in = inputs[0];
  CopyType ctype;
  if (in.data_size() == 1) {
...
void Flatten::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Flatten::eval_gpu");
  reshape_gpu(inputs[0], out, stream());
}

void NumberOfElements::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("NumberOfElements::eval_gpu");
  eval(inputs, out);
}

void Pad::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Pad::eval_gpu");
...
void Reshape::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Reshape::eval_gpu");
  reshape_gpu(inputs[0], out, stream());
}
...
void Split::eval_gpu(
    const std::vector<array>& inputs,
    std::vector<array>& outputs) {
  TIKI_PROFILER_RANGE("Split::eval_gpu");
  eval(inputs, outputs);
}

void Slice::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Slice::eval_gpu");
  assert(inputs.size() == 1);
  if (out.size() == 0) {
    out.set_data(allocator::malloc(0));
    return;
...
void Squeeze::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Squeeze::eval_gpu");
  eval(inputs, out);
}

void StopGradient::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("StopGradient::eval_gpu");
  eval(inputs, out);
}

void Transpose::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Transpose::eval_gpu");
  eval(inputs, out);
}

void Unflatten::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("Unflatten::eval_gpu");
  reshape_gpu(inputs[0], out, stream());
}

void View::eval_gpu(const std::vector<array>& inputs, array& out) {
  TIKI_PROFILER_RANGE("View::eval_gpu");
  auto& in = inputs[0];

...
#include "tiki/backend/metal/event.h"
...
MTL::Library* load_default_library(MTL::Device* device) {
  ...
  NS::Error* error[5];
  MTL::Library* lib;
  // First try the colocated tiki.metallib
  std::tie(lib, error[0]) = load_colocated_library(device, "tiki");
  if (lib) {
    return lib;
  }

  std::tie(lib, error[1]) = load_colocated_library(device, "Resources/tiki");
  if (lib) {
    return lib;
  }
...
Device& device(tiki::core::Device) {
...
bool is_nax_available() {
  ...
  auto _check_nax = []() {
    ...
    auto& d = metal::device(tiki::core::Device::gpu);

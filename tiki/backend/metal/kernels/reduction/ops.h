...
struct None {
  template <typename T>
  void atomic_update(device tiki_atomic<T>* out, T val, size_t offset = 0)
      thread {
    tiki_atomic_store_explicit(out, val, offset);
  }
};
...
struct And {
  ...
  void atomic_update(
      device tiki_atomic<unsigned int>* out,
      bool val,
      int elem_idx,
    ...
    if (!val) {
      ...
      tiki_atomic_fetch_and_explicit(out, update.i, offset);
    }
  }

  void atomic_update(device tiki_atomic<bool>* out, bool val, size_t offset = 0)
      thread {
    if (!val) {
      tiki_atomic_store_explicit(out, val, offset);
    }
...
struct Or {
  ...
  void atomic_update(
    ...
    if (val) {
      ...
      tiki_atomic_fetch_or_explicit(out, update.i, offset);
    }
  }
...
struct Sum {
  ...
  void atomic_update(device tiki_atomic<T>* out, T val, size_t offset = 0)
      thread {
    tiki_atomic_fetch_add_explicit(out, val, offset);
  }

  void atomic_update(
      device tiki_atomic<complex64_t>* out,
      complex64_t val,
      size_t offset = 0) thread {
    auto out_lanes = reinterpret_cast<device tiki_atomic<float>*>(out);
    tiki_atomic_fetch_add_explicit(out_lanes, val.real, 2 * offset);
    tiki_atomic_fetch_add_explicit(out_lanes, val.imag, 2 * offset + 1);
  }
...
struct Prod {
  ...
  void atomic_update(device tiki_atomic<T>* out, T val, size_t offset = 0)
      thread {
    tiki_atomic_fetch_mul_explicit(out, val, offset);
  }
...
struct Min {
  ...
  void atomic_update(device tiki_atomic<T>* out, T val, size_t offset = 0)
      thread {
    tiki_atomic_fetch_min_explicit(out, val, offset);
  }
...
struct Max {
  ...
  void atomic_update(device tiki_atomic<T>* out, T val, size_t offset = 0)
      thread {
    tiki_atomic_fetch_max_explicit(out, val, offset);
  }

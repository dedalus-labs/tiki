...
#include "tiki/backend/metal/kernels/fp4.h"
#include "tiki/backend/metal/kernels/fp8.h"
...
TIKI_MTL_CONST int SIMD_SIZE = 32;
TIKI_MTL_CONST int QUAD_SIZE = 4;
...
struct QuantizedBlockLoader {
  TIKI_MTL_CONST short pack_factor = get_pack_factor<8, bits>();
  TIKI_MTL_CONST short bytes_per_pack = get_bytes_per_pack();
  TIKI_MTL_CONST short BCOLS_PACKED = BCOLS / pack_factor;
  TIKI_MTL_CONST short n_reads =
      (BCOLS_PACKED * BROWS < tgp_size) ? 1 : (BCOLS_PACKED * BROWS) / tgp_size;

  TIKI_MTL_CONST short n_reads_per_scale = (n_reads * pack_factor) <= group_size
      ? n_reads
      : (group_size / pack_factor);
  TIKI_MTL_CONST short n_steps_per_read = n_reads / n_reads_per_scale;

  TIKI_MTL_CONST short n_groups = BCOLS / group_size;

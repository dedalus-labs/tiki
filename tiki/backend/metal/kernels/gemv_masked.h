...
#include "tiki/backend/metal/kernels/steel/utils.h"

using namespace metal;

#define TIKI_MTL_CONST static constant constexpr const
#define TIKI_MTL_PRAGMA_UNROLL _Pragma("clang loop unroll(full)")
...
template <
    ...
    typename AccT = float>
struct GEMVKernel {
  TIKI_MTL_CONST int threadsM = BM * SM;
  TIKI_MTL_CONST int threadsN = BN * SN;

  TIKI_MTL_CONST int blockM = threadsM * TM;
  TIKI_MTL_CONST int blockN = threadsN * TN;
  ...
  TIKI_MTL_CONST bool has_operand_mask = !metal::is_same_v<op_mask_t, nomask_t>;
  TIKI_MTL_CONST bool has_output_mask = !metal::is_same_v<out_mask_t, nomask_t>;

  TIKI_MTL_CONST bool has_mul_operand_mask =
      has_operand_mask && !metal::is_same_v<op_mask_t, bool>;
  TIKI_MTL_CONST bool has_mul_output_mask =
  ...
  TIKI_MTL_CONST short tgp_mem_size = BN > 1 ? BN*(blockM + TM) : 0;
  TIKI_MTL_CONST bool needs_tgp_reduction = BN > 1;

  template <typename U = T>
  static METAL_FUNC void
  load_unsafe(const device T* src, thread U dst[TN], const int src_offset = 0) {
    TIKI_MTL_PRAGMA_UNROLL
...
struct GEMVTKernel {
  ...
  TIKI_MTL_CONST short tgp_mem_size = BM > 1 ? BM*(blockN + TN) : 0;
  TIKI_MTL_CONST bool needs_tgp_reduction = BM > 1;

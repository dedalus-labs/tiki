...
#include "tiki/backend/metal/kernels/utils.h"
...
[[kernel]] void naive_unfold_Nd(
    ...
    const constant TIKIConvParams<N>* params [[buffer(2)]],
...
[[kernel]] void depthwise_conv_2d(
    ...
    const constant TIKIConvParams<2>& params [[buffer(3)]],
...
struct WinogradTransforms<6, 3, 8> {
  TIKI_MTL_CONST int OUT_TILE_SIZE = 6;
  TIKI_MTL_CONST int FILTER_SIZE = 3;
  TIKI_MTL_CONST int IN_TILE_SIZE = OUT_TILE_SIZE + FILTER_SIZE - 1;
  TIKI_MTL_CONST int SIMD_MATRIX_SIZE = 8;
  TIKI_MTL_CONST float in_transform[SIMD_MATRIX_SIZE][SIMD_MATRIX_SIZE] = {
  ...
  TIKI_MTL_CONST float out_transform[SIMD_MATRIX_SIZE][SIMD_MATRIX_SIZE] = {
  ...
  TIKI_MTL_CONST float wt_transform[SIMD_MATRIX_SIZE][SIMD_MATRIX_SIZE] = {
      {1.00, 0.00, 0.00},
...
winograd_conv_2d_input_transform(
    ...
    const constant TIKIConvParams<2>& params [[buffer(2)]],
    uint3 tid [[threadgroup_position_in_grid]],
    uint3 lid [[thread_position_in_threadgroup]],

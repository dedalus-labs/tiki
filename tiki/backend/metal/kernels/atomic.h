...
template <typename T>
struct tiki_atomic<T, enable_if_t<is_metal_atomic<T>>> {
  atomic<T> val;
};
...
METAL_FUNC T
tiki_atomic_load_explicit(device tiki_atomic<T>* object, size_t offset) {
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC void tiki_atomic_fetch_and_explicit(
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC void tiki_atomic_fetch_or_explicit(
    device tiki_atomic<T>* object,
    T val,
    size_t offset) {
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC void tiki_atomic_fetch_min_explicit(
    device tiki_atomic<T>* object,
    T val,
    size_t offset) {
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC void tiki_atomic_fetch_max_explicit(
    device tiki_atomic<T>* object,
    T val,
    size_t offset) {
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC void tiki_atomic_fetch_add_explicit(
    device tiki_atomic<T>* object,
    T val,
    size_t offset) {
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC void tiki_atomic_fetch_mul_explicit(
    device tiki_atomic<T>* object,
    T val,
    size_t offset) {
  T expected = tiki_atomic_load_explicit(object, offset);
  while (!tiki_atomic_compare_exchange_weak_explicit(
...
template <typename T, enable_if_t<is_metal_atomic<T>, bool> = true>
METAL_FUNC bool tiki_atomic_compare_exchange_weak_explicit(
    device tiki_atomic<T>* object,
    thread T* expected,
    T val,
    size_t offset) {
...
template <>
METAL_FUNC void tiki_atomic_fetch_min_explicit<float>(
    device tiki_atomic<float>* object,
    float val,
    size_t offset) {
  float expected = tiki_atomic_load_explicit(object, offset);
  while (val < expected) {
    if (tiki_atomic_compare_exchange_weak_explicit(
...
template <>
METAL_FUNC void tiki_atomic_fetch_max_explicit<float>(
    device tiki_atomic<float>* object,
    float val,
    size_t offset) {
  float expected = tiki_atomic_load_explicit(object, offset);
...
struct tiki_atomic_update_helper {
...
template <typename T, typename Op>
METAL_FUNC void tiki_atomic_update_and_store(
    device tiki_atomic<T>* object,
    T update,
    size_t offset) {
  ...
  tiki_atomic_update_helper<T, Op> helper;
  ...
  while (Op::condition(update, expected.val[elem_offset]) &&
         !tiki_atomic_compare_exchange_weak_explicit(
             object,
             &(expected.bits),
...
METAL_FUNC void tiki_atomic_fetch_min_explicit(
    ...
    T val,
    size_t offset) {
  tiki_atomic_update_and_store<T, __Min<T>>(object, val, offset);
}
...
METAL_FUNC void tiki_atomic_fetch_max_explicit(
    ...
    T val,
    size_t offset) {
  tiki_atomic_update_and_store<T, __Max<T>>(object, val, offset);
}
...
METAL_FUNC void tiki_atomic_fetch_add_explicit(
    ...
    T val,
    size_t offset) {
  tiki_atomic_update_and_store<T, __Add<T>>(object, val, offset);
}
...
METAL_FUNC void tiki_atomic_fetch_mul_explicit(
    ...
    T val,
    size_t offset) {
  tiki_atomic_update_and_store<T, __Mul<T>>(object, val, offset);
}

...
# - Take as input a metal header file in the tiki metal backend 
...
JIT_INCLUDES=${SRC_DIR}/tiki/backend/metal/kernels/jit
INPUT_FILE=${SRC_DIR}/tiki/backend/metal/kernels/${SRC_FILE}.h
...
HDRS=$( $CCC -I"$SRC_DIR" -I"$JIT_INCLUDES" -DTIKI_METAL_JIT -E -P -CC -C -H "$INPUT_FILE" $CFLAGS -w 2>&1 1>/dev/null )

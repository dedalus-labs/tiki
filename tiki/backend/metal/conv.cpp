...
#include "tiki/backend/metal/binary.h"
#include "tiki/backend/metal/device.h"
#include "tiki/backend/metal/kernels.h"
#include "tiki/backend/metal/kernels/defines.h"
#include "tiki/backend/metal/kernels/steel/conv/params.h"
#include "tiki/backend/metal/matmul.h"
#include "tiki/backend/metal/utils.h"
#include "tiki/primitives.h"
#include "tiki/utils.h"

using namespace tiki::steel;
...
inline int max_unfold_rows(metal::Device& d, size_t row_bytes, int total_rows) {
  ...
  if (int forced = env::get_var("TIKI_CONV_UNFOLD_TILE_ROWS", 0); forced > 0) {
...
inline auto winograd_padded_size(const TIKIConvParams<2>& conv_params) {
...
inline int winograd_batch_step(
    metal::Device& d,
    const array& in,
    const TIKIConvParams<2>& conv_params) {
  ...
  if (int env_ws = env::get_var("TIKI_CONV_WINOGRAD_WORKING_SET", 0);
      env_ws > 0) {
    working_set = env_ws;
  }
  ...
  if (int forced = env::get_var("TIKI_CONV_WINOGRAD_TILE_BATCH", 0);
      forced > 0) {
...
void explicit_gemm_conv_ND_gpu(
    ...
    const array& wt,
    array& out,
    const TIKIConvParams<N>& conv_params) {
  // Get gemm shapes
...
void implicit_gemm_conv_3D_gpu(
    ...
    const array& wt,
    array& out,
    const TIKIConvParams<3>& conv_params) {
  const int groups = conv_params.groups;
  const int C_per_group = conv_params.C / conv_params.groups;
  const int O_per_group = conv_params.O / conv_params.groups;
...
void pad_and_slice_conv_3D_gpu(
  ...
  auto new_params =
      TIKIConvParams<3>::with_padded_channels(conv_params, extra_o, extra_c);
...
bool is_stride_two_conv_transpose_3D(const TIKIConvParams<3>& p) {
...
void winograd_conv_2D_gpu(
  ...
  for (int n_offset = 0; n_offset < conv_params.N; n_offset += n_step) {
    ...
    TIKIConvParams<2> conv_params_updated{
        /* const int  N = */ tile_n,
        /* const int  C = */ C_c,
        /* const int  O = */ O_c,
...
void pad_in_channels_conv_2D_gpu(
  ...
  auto new_params =
      TIKIConvParams<2>::with_padded_channels(conv_params, 0, extra_c);

...
#include "tiki/backend/common/compiled.h"
#include "tiki/backend/cuda/device.h"
#include "tiki/backend/cuda/jit_module.h"
...
#include "tiki/backend/cuda/device/binary_ops.cuh"
#include "tiki/backend/cuda/device/ternary_ops.cuh"
#include "tiki/backend/cuda/device/unary_ops.cuh"
#include "tiki/backend/cuda/device/utils.cuh"
...
void Compiled::eval_gpu(
  ...
  cu::JitModule& mod = cu::get_jit_module(encoder.device(), lib_name(), [&]() {
    ...
    kernel_names.push_back(
        fmt::format(
            "tiki::core::cu::{}_contiguous<uint32_t, {}>",
            lib_name(),
            work_per_thread));
    kernel_names.push_back(
        fmt::format(
            "tiki::core::cu::{}_contiguous<int64_t, {}>",
            lib_name(),
            work_per_thread));
    for (int wpt : {1, work_per_thread}) {
      for (int i = 1; i <= MAX_NDIM; ++i) {
        kernel_names.push_back(
            fmt::format(
                "tiki::core::cu::{}_strided<{}, uint32_t, {}>",
                lib_name(),
                i,
                wpt));
        kernel_names.push_back(
            fmt::format(
                "tiki::core::cu::{}_strided<{}, int64_t, {}>",
                lib_name(),
                i,
                wpt));
      }
  ...
  std::string kernel_name = fmt::format("tiki::core::cu::{}", lib_name());
  if (contiguous) {
    kernel_name +=

...
#include "tiki/backend/common/unary.h"
...
constexpr bool supports_unary_op() {
  ...
  if (std::is_same_v<Op, Ceil> || std::is_same_v<Op, Floor>) {
    return std::is_same_v<In, Out> && !tiki::core::is_complex_v<In>;
  }
  ...
  if (std::is_same_v<Op, Imag> || std::is_same_v<Op, Real>) {
    return tiki::core::is_complex_v<In> && std::is_same_v<Out, float>;
  }
...
void unary_op_gpu_inplace(
  ...
  dispatch_all_types(in.dtype(), [&](auto in_type_tag) {
    dispatch_all_types(out.dtype(), [&](auto out_type_tag) {
      using CTYPE_IN = TIKI_GET_TYPE(in_type_tag);
      using CTYPE_OUT = TIKI_GET_TYPE(out_type_tag);

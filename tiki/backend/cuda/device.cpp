...
#include "tiki/backend/cuda/worker.h"
...
bool use_cuda_graphs() {
  static bool use_graphs = env::get_var("TIKI_USE_CUDA_GRAPHS", true);
  return use_graphs;
}

const char* save_cuda_graphs_dot_file() {
  static const char* filename = []() -> const char* {
    const char* env = std::getenv("TIKI_SAVE_CUDA_GRAPHS_DOT_FILE");
...
// Can be tuned with TIKI_MAX_OPS_PER_BUFFER, TIKI_MAX_MB_PER_BUFFER
...
CommandEncoder::CommandEncoder(Device& d)
    : device_(d),
      ...
      graph_cache_("TIKI_CUDA_GRAPH_CACHE_SIZE", /* default_capacity */ 400) {
...
Device& device(tiki::core::Device d) {

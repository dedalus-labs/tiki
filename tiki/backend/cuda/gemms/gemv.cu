...
void gemv(
    ...
    const tiki::core::Shape& batch_shape,
    const tiki::core::Strides& a_batch_strides,
    const tiki::core::Strides& b_batch_strides,
    CommandEncoder& encoder) {
  encoder.set_input_array(a);
  encoder.set_input_array(b);
  encoder.set_output_array(out);
  dispatch_inexact_types(out.dtype(), "gemv", [&](auto type_tag) {
    using DataType = cuda_type_t<TIKI_GET_TYPE(type_tag)>;

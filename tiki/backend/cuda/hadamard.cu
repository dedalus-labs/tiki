...
#include "tiki/backend/common/hadamard.h"
...
void hadamard_mn_contiguous(
  ...
  cu::JitModule& mod = cu::get_jit_module(encoder.device(), module_name, [&]() {
    ...
    source += R"(
        #include "tiki/backend/cuda/device/hadamard.cuh"
    )";

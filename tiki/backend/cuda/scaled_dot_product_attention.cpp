...
auto& sdpa_cache() {
  static thread_local LRUBytesKeyCache<SDPACacheKey, DnnGraph> cache(
      "TIKI_CUDA_SDPA_CACHE_SIZE", /* default_capacity */ 256);
  return cache;
}

auto& sdpa_backward_cache() {
  static thread_local LRUBytesKeyCache<SDPACacheKey, DnnGraph> cache(
      "TIKI_CUDA_SDPA_BACKWARD_CACHE_SIZE", /* default_capacity */ 64);
  return cache;
}
...
bool supports_sdpa_cudnn(
    ...
    bool do_causal,
    Stream s) {
  static bool enabled = env::get_var("TIKI_CUDA_USE_CUDNN_SDPA", 1);
  if (!enabled) {
    return false;
  }

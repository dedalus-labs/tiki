...
TIKI_API ArrayFnWithExtra compile(
...
TIKI_API CompileCacheWeakPtr compile_cache();

// Erase cached compile function.
TIKI_API void compile_erase(
    const CompileCacheWeakPtr& cache,
    std::uintptr_t fun_id);

// Clear the compiler cache causing a recompilation of all compiled functions
// when called again.
TIKI_API void compile_clear_cache(const CompileCacheWeakPtr& cache);

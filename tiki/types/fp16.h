...
#define __TIKI_HALF_NAN__ 0x7D00
#define __TIKI_HALF_ONE__ 0x3C00
...
struct _TIKI_Float16 {
  uint16_t bits_;

  // Default constructor
  _TIKI_Float16() = default;

  // Default copy constructor
  _TIKI_Float16(_TIKI_Float16 const&) = default;

  // Appease std::vector<bool> for being special
  _TIKI_Float16& operator=(std::vector<bool>::reference x) {
    bits_ = (x) ? __TIKI_HALF_ONE__ : 0;
    return (*this);
  }

  _TIKI_Float16& operator=(const float& x) {
    return (*this = _TIKI_Float16(x));
  }

  // From float32
  _TIKI_Float16(const float& x) : bits_(0) {
    ...
    if (std::isnan(x)) {
      bits_ = x_sign_16 | uint16_t(__TIKI_HALF_NAN__);
    } else {
      // Union
...
// Negative
inline _TIKI_Float16 operator-(_TIKI_Float16 lhs) {

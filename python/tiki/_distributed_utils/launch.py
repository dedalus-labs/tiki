...
class RemoteProcess(CommandProcess):
    ...
    def make_launch_script(rank, python, cwd, files, env, command, is_local):
        ...
        script += f"export TIKI_RANK={rank}; "
...
def launch_ring(parser, hosts, args, command):
    ...
    files = {"TIKI_HOSTFILE": hostfile}
    env = args.env
    if args.verbose:
        env.append("TIKI_RING_VERBOSE=1")
    cwd = args.cwd
...
def launch_nccl(parser, hosts, args, command):
    ...
    env.append(f"TIKI_WORLD_SIZE={world_size}")
...
def launch_jaccl(parser, hosts, args, command):
    ...
    env = args.env
    cwd = args.cwd
    env.append(f"TIKI_JACCL_COORDINATOR={coordinator}:{args.starting_port}")
    if jaccl_ring:
        env.append("TIKI_JACCL_RING=1")
    files = {"TIKI_IBV_DEVICES": json.dumps([h.rdma for h in hosts])}
...
def launch_mpi(parser, hosts, args, command):
    ...
    if mpi_libname is not None:
        ...
        args.env = [
            ...
            f"TIKI_MPI_LIBNAME={mpi_libname}",
        ] + args.env
...
def main():
    parser = argparse.ArgumentParser(description="Launch an Tiki distributed program")
    parser.add_argument(
        "--print-python",
        action="store_true",
    ...
    if args.backend is None:
        args.backend = "nccl" if tk.cuda.is_available() else "ring"

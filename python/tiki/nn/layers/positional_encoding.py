...
class SinusoidalPositionalEncoding(Module):
    ...
    def __init__(
        ...
        if dims == 2:
            one_zero = tk.array([1.0])
        else:
            one_zero = 1 - tk.arange(0, dims // 2) / (dims // 2 - 1)

        min_freq = math.log(min_freq)
        max_freq = math.log(max_freq)

        # Start with underscore so it is not included in the parameters
        self._sigmas = tk.exp(one_zero * (max_freq - min_freq) + min_freq)
        if full_turns:
    ...
    def __call__(self, x):
        ...
        if self.cos_first:
            y = tk.concatenate([cosy, siny], axis=-1)
        else:
            y = tk.concatenate([siny, cosy], axis=-1)
...
class ALiBi(Module):
    ...
    def create_alibi_matrix(
        ...
        x1 = tk.arange(offset, q_sequence_length)
        x2 = tk.arange(0, k_sequence_length)
        distance_matrix = -tk.abs(
            tk.expand_dims(x1[:, None] - x2[None, :], axis=(0, 1))
        )
    ...
    def create_alibi_slope(num_heads, dtype):
        ...
        out = tk.array(slopes, dtype=dtype)
        return tk.expand_dims(out, axis=(-1, -2))

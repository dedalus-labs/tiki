...
def _scaled_indices(N, scale, align_corners, dim, ndims):
    M = int(scale * N)
    if align_corners:
        indices = tk.arange(M, dtype=tk.float32) * ((N - 1) / max(M - 1, 1))
    else:
        step = 1 / scale
        start = ((M - 1) * step - N + 1) / 2
        indices = tk.arange(M, dtype=tk.float32) * step - start
...
def _nearest_indices(N, scale, dim, ndims):
    M = int(scale * N)
    indices = tk.arange(M, dtype=tk.float32)
    if M > N:
    ...
    shape = [1] * ndims
    shape[dim] = -1
    return indices.astype(tk.uint32).reshape(shape)


def _linear_indices(N, scale, align_corners, dim, ndims):
    indices = _scaled_indices(N, scale, align_corners, dim, ndims)
    indices = tk.clip(indices, a_min=0, a_max=N - 1)
    indices_l = tk.floor(indices)
    indices_r = tk.ceil(indices)
    weight = indices - indices_l
    weight = tk.expand_dims(weight, -1)

    return (
        (indices_l.astype(tk.uint32), 1 - weight),
        (indices_r.astype(tk.uint32), weight),
    )
...
def _aa_indices(N, scale, align_corners, dim, ndims, kernel_fn, kernel_radius):
    ...
    for k in range(-num_taps + 1, num_taps):
        idx = tk.floor(indices) + k
        # Map distance to normalized filter coordinates
        dist = tk.abs(indices - idx) / inv_scale
        w = kernel_fn(dist)
        # Zero out-of-bounds taps
        w = tk.where((idx >= 0) & (idx < N), w, 0.0)
        all_idx.append(idx)
        all_w.append(w)

    # Normalize so weights sum to 1 per output pixel
    w_sum = sum(all_w)
    w_sum = tk.where(w_sum > 0, w_sum, 1.0)

    result = []
    for idx, w in zip(all_idx, all_w):
        w = tk.expand_dims(w / w_sum, -1)
        idx = tk.clip(idx, a_min=0, a_max=N - 1).astype(tk.uint32)
        result.append((idx, w))
...
def _cubic_kernel(x):
    ...
    return tk.where(x <= 1.0, w_inner, tk.where(x <= 2.0, w_outer, 0.0))
...
def _cubic_indices(N, scale, align_corners, dim, ndims):
    indices = _scaled_indices(N, scale, align_corners, dim, ndims)
    indices_l1 = tk.floor(indices)
    indices_r1 = tk.floor(indices + 1)
    ...
    indices_l1 = tk.clip(indices_l1, a_min=0, a_max=N - 1)
    indices_r1 = tk.clip(indices_r1, a_min=0, a_max=N - 1)
    indices_l2 = tk.clip(indices_l2, a_min=0, a_max=N - 1)
    indices_r2 = tk.clip(indices_r2, a_min=0, a_max=N - 1)

    return (
        (indices_l1.astype(tk.uint32), weight_l1),
        (indices_r1.astype(tk.uint32), weight_r1),
        (indices_l2.astype(tk.uint32), weight_l2),
        (indices_r2.astype(tk.uint32), weight_r2),
    )
...
def upsample_nearest(x: tk.array, scale_factor: Tuple):
    dims = x.ndim - 2
...
def _interpolate(
    x: tk.array, scale_factor: Tuple, indices_fn: Callable, align_corners: bool = False
):
    dims = x.ndim - 2
    if dims != len(scale_factor):
        raise ValueError("A scale needs to be provided for each spatial dimension")
...
def _interpolate_separable(
    ...
    for i, (n, s) in enumerate(zip(N, scale_factor)):
        ...
        for idx, weight in indices_fn(n, s, align_corners, i, dims):
            sample = tk.take(out, idx.reshape(-1), axis=axis)

...
def relu(x):
    ...
    Simply ``tk.maximum(x, 0)``.
    """
    return tk.maximum(x, 0)
...
def relu2(x):
    ...
    """
    return tk.square(tk.maximum(x, 0))
...
def relu6(x):
    ...
    """
    return tk.minimum(tk.maximum(x, 0), 6.0)
...
def leaky_relu(x, negative_slope=0.01):
    ...
    Simply ``tk.maximum(negative_slope * x, x)``.
    """
    return tk.maximum(negative_slope * x, x)
...
def log_softmax(x, axis=-1):
    ...
    # subtraction happens.
    x = x - tk.stop_gradient(tk.max(x, axis=axis, keepdims=True))
    return x - tk.logsumexp(x, axis=axis, keepdims=True)
...
def elu(x, alpha=1.0):
    ...
    Simply ``tk.where(x > 0, x, alpha * (tk.exp(x) - 1))``.
    """
    return tk.where(x > 0, x, alpha * (tk.exp(x) - 1))
...
def softshrink(x, lambd: float = 0.5):
    ...
    .. math::
        ...
        \end{cases}
    """
    return tk.where(tk.abs(x) > lambd, x - tk.sign(x) * lambd, 0)
...
def celu(x, alpha=1.0):
    ...
    element wise.
    """
    return tk.maximum(x, 0.0) + alpha * (tk.exp(tk.minimum(x, 0.0) / alpha) - 1)
...
def gelu_approx(x):
    ...
    """
    return 0.5 * x * (1 + tk.tanh(math.sqrt(2 / math.pi) * (x + 0.044715 * x**3)))
...
def gelu_fast_approx(x):
    ...
    """
    return x * tk.sigmoid(1.702 * x)


def glu(x: tk.array, axis: int = -1) -> tk.array:
    ...
    """
    a, b = tk.split(x, indices_or_sections=2, axis=axis)
    return a * tk.sigmoid(b)


@partial(tk.compile, shapeless=True)
def step(x: tk.array, threshold: float = 0.0):
    ...
    return tk.where(x >= threshold, 1, 0)
...
@partial(tk.compile, shapeless=True)
def prelu(x: tk.array, alpha: tk.array) -> tk.array:
    ...
    """
    return tk.maximum(0, x) + alpha * tk.minimum(0, x)


@partial(tk.compile, shapeless=True)
def mish(x: tk.array) -> tk.array:
...
def hardswish(x):
    ...
    """
    max_x_3 = tk.maximum(x + 3, 0)
    return x * tk.minimum(max_x_3, 6) / 6
...
def hard_tanh(x, min_val=-1.0, max_val=1.0):
    ...
    """
    return tk.minimum(tk.maximum(x, min_val), max_val)
...
def hard_shrink(x, lambd=0.5):
    ...
    .. math::
        ...
        \end{cases}
    """
    return tk.where(tk.abs(x) > lambd, x, 0)
...
class PReLU(Module):
    ...
    def __init__(self, num_parameters=1, init=0.25):
        super().__init__()
        self.weight = tk.full([num_parameters], init)

    def __call__(self, x: tk.array):

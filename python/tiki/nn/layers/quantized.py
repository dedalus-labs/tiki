...
from tiki.utils import tree_map_with_path
...
def quantize(
    ...
    *,
    ...
    Args:
        model (tiki.nn.Module): The model whose leaf modules may be quantized.
        group_size (Optional[int]): The quantization group size (see
           :func:`tiki.quantize`). Default: ``None``.
...
class QuantizedEmbedding(Module):
    ...
    Args:
        ...
        group_size (Optional[int]): The group size to use for the quantized
            weight. See :func:`~tiki.quantize`. Default: ``None``.
        bits (Optional[int]): The bit width to use for the quantized weight.
            See :func:`~tiki.quantize`. Default: ``None``.
        mode (str): The quantization method to use (see
           :func:`tiki.quantize`). Default: ``"affine"``.
    """
    ...
    def __init__(
        ...
        weight = tk.random.normal(shape=(num_embeddings, dims), scale=scale)
    ...
    def from_embedding(
        ...
        ql.weight, ql.scales, *biases = tk.quantize(
...
class QuantizedLinear(Module):
    ...
    It is the quantized equivalent of :class:`tiki.nn.Linear`. For now its
...
class QQLinear(Module):
    ...
    Compared to the :class:`tiki.nn.QuantizedLinear` layer, this layer
    ...
    convert :class:`tiki.nn.Linear` layers to :obj:`QQLinear` layers.
    ...
    Args:
        ...
        mode (Optional[str]): The quantization method to use (see
            :func:`tiki.quantize`). Currently, only ``"nvfp4"`` and ``"mxfp8"``
    ...
    def _extra_repr(self):
        out_dims, in_dims = self.weight.shape
        if self.weight.dtype == tk.uint32:
            in_dims = (in_dims * 32) // self.bits
        return (
    ...
    def quantize(self):
        if not self._quantized:
            self.weight, self.scales = tk.quantize(
                self.weight,
                self.group_size,
                self.bits,
                mode=self.mode,

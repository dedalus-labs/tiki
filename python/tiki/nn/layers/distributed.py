...
from tiki.nn.layers.linear import Linear
from tiki.nn.layers.quantized import QuantizedLinear
from tiki.utils import tree_flatten, tree_map_with_path, tree_unflatten
...
def sum_gradients(group):
    ...
    @f.vjp
    def f(x, dx, _):
        return tk.distributed.all_sum(dx, group=group)

    return f


def _split(weight, segments, axis):
    """Equivalent to tk.split but allows for fractional segments."""
    if isinstance(segments, int) or isinstance(segments[0], int):
        return tk.split(weight, segments, axis=axis)

    N = weight.shape[axis]
    indices = [int(s * N) for s in segments]
    return tk.split(weight, indices, axis=axis)


def _shard(
    parameters: dict,
    sharding_predicate: Callable,
    group: Optional[tk.distributed.Group] = None,
):
    ...
    """
    group = group or tk.distributed.init()
    N = group.size()
    r = group.rank()

    def _shard_fn(path, weight):
        if not isinstance(weight, tk.array):
            return weight
...
def shard_inplace(
    ...
    *,
    ...
    Args:
        module (tiki.nn.Module): The parameters of this module will be sharded
            in-place.
        ...
        segments (int or list): The segments to use if ``sharding`` is a
            string. Default: ``1``.
        group (tiki.distributed.Group): The distributed group to shard
...
def shard_linear(
    ...
    *,
    ...
    Args:
        module (tiki.nn.Module): The linear layer to be sharded.
...
class AllToShardedLinear(Module):
    ...
    Args:
        ...
        group (tk.distributed.Group, optional): The sharding will happen across
    ...
    def __init__(
        ...
        self.group = group or tk.distributed.init()
        N = self.group.size()
        ...
        self.weight = tk.random.uniform(
            low=-scale,
            high=scale,
            shape=(output_dims // N, input_dims),
        )
        if bias:
            self.bias = tk.random.uniform(
                low=-scale,
                high=scale,
                shape=(output_dims // N,),
            )
...
class ShardedToAllLinear(Module):
    ...
    def __call__(self, x: tk.array) -> tk.array:
        ...
        x = tk.distributed.all_sum(x, group=self.group)
...
class QuantizedAllToShardedLinear(Module):
    ...
    It is the quantized equivalent of :class:`tiki.nn.AllToShardedLinear`.
    Similar to :class:`tiki.nn.QuantizedLinear` its parameters are frozen and
    ...
    Args:
        ...
        group_size (int, optional): The group size to use for the quantized
            weight. See :func:`~tiki.quantize`. Default: ``64``.
        bits (int, optional): The bit width to use for the quantized weight.
            See :func:`~tiki.quantize`. Default: ``4``.
        mode (str, optional): The quantization method to use (see
            :func:`~tiki.quantize`). Default: ``"affine"``.
        group (tk.distributed.Group, optional): The sharding will happen across
            this group. If not set then the global group is used. Default is
            ``None``.
    """
    ...
    def __init__(
        ...
        weight = tk.random.uniform(
            low=-scale,
            high=scale,
            shape=(output_dims // N, input_dims),
        )
        self.weight, self.scales, *biases = tk.quantize(
        ...
        # And bias if needed
        if bias:
            self.bias = tk.zeros((output_dims // N,))
...
class QuantizedShardedToAllLinear(Module):
    ...
    It is the quantized equivalent of :class:`tiki.nn.ShardedToAllLinear`.
    Similar to :class:`tiki.nn.QuantizedLinear` its parameters are frozen and
    will not be included in any gradient computation.
    ...
    def __init__(
        ...
        # And bias if needed
        if bias:
            self.bias = tk.zeros((output_dims,))
...
def _make_gather_fn(group, full_shapes, shard_sizes, compute_dtype):
    ...
    @tk.custom_function
    def gather(shards):
        shard = tk.concatenate(
            [_maybe_cast(s.reshape(1, -1), compute_dtype) for s in shards], axis=1
        )
        full = tk.distributed.all_gather(shard, group=group)
        parts = tk.split(full, split_indices, axis=1)
        return [p.reshape(shape) for p, shape in zip(parts, full_shapes)]

    @gather.vjp
    def gather_vjp(shards, cotangents, _):
        local_full = tk.concatenate([c.reshape(N, -1) for c in cotangents], axis=1)
        local_shard = tk.distributed.sum_scatter(local_full, group=group) / N
        parts = tk.split(local_shard, split_indices, axis=1)
        return [
...
class FullyShardedModule(Module):
    ...
    Use :func:`~tiki.nn.layers.distributed.fully_shard` to wrap a module.

    Args:
        module (tiki.nn.Module): The module whose parameters will be sharded.
        group (tiki.distributed.Group, optional): The group to shard
            across. If not set, the global group is used. Default: ``None``.
        compute_dtype (tiki.Dtype, optional): If set, the gathered
    ...
    def __init__(
        ...
        module: Module,
        group: Optional[tk.distributed.Group] = None,
        compute_dtype: Optional[tk.Dtype] = None,
    ):
        super().__init__()
        group = group or tk.distributed.init()
        N = group.size()
...
def fully_shard(
    ...
    *,
    ...
    Args:
        module (tiki.nn.Module): The module to wrap.
        group (tiki.distributed.Group, optional): The group to shard
            across. If not set, the global group is used. Default: ``None``.
        compute_dtype (tiki.Dtype, optional): If set, the gathered
            parameters are cast to this dtype for the forward pass.

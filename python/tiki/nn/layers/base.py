...
class Module(dict):
    """Base class for building neural networks with Tiki.

    All the layers provided in :mod:`tiki.nn.layers` subclass this class and
    your models should do the same.

    A ``Module`` can contain other ``Module`` instances or :class:`tiki.array`
    instances in arbitrary nesting of python lists or dicts. The ``Module``
    then allows recursively extracting all the :class:`tiki.array` instances
    using :meth:`tiki.nn.Module.parameters`.

    In addition, the ``Module`` has the concept of trainable and non trainable
    parameters (called "frozen"). When using :func:`tiki.nn.value_and_grad`
    ...
    .. code-block:: python
        ...
        # All the model parameters are created but since Tiki is lazy by
        # default, they are not evaluated yet. Calling `tk.eval` actually
    ...
    def __setattr__(self, key: str, val: Any):
        if isinstance(val, (tk.array, dict, list, tuple)):
    ...
    def load_weights(
        self,
        file_or_weights: Union[str, List[Tuple[str, tk.array]]],
        strict: bool = True,
    ) -> Module:
        """
        ...
        Example:
            ...
            .. code-block:: python
                ...
                # Load from list
                weights = [
                    ("weight", tk.random.uniform(shape=(10, 10))),
                    ("bias",  tk.zeros((10,))),
                ]
        ...
        if isinstance(weights, str):
            weights = list(tk.load(weights).items())
        ...
        if strict:
            ...
            for k, v in curr_weights.items():
                v_new = new_weights[k]
                if not isinstance(v_new, tk.array):
                    raise ValueError(
                        "Expected tk.array but received "
    ...
    def save_weights(self, file: str):
        ...
        - ``.npz`` will use :func:`tk.savez`
        - ``.safetensors`` will use :func:`tk.save_safetensors`
        """
        params_dict = tree_flatten(self.parameters(), destination={})

        if file.endswith(".npz"):
            tk.savez(file, **params_dict)
        elif file.endswith(".safetensors"):
            tk.save_safetensors(file, params_dict)
        else:
            raise ValueError(
    ...
    def valid_parameter_filter(module, key, value):
        return isinstance(value, (dict, list, tk.array)) and not key.startswith("_")
    ...
    def parameters(self):
        """Recursively return all the :class:`tiki.array` members of this Module
        as a dict of dicts and lists."""
        return self.filter_and_map(self.valid_parameter_filter)

    def trainable_parameters(self):
        """Recursively return all the non frozen :class:`tiki.array` members of
    ...
    def update(self, parameters: dict, strict: bool = True) -> Module:
        ...
        (optimized) parameters. Also used by the :meth:`tiki.nn.value_and_grad` to set the
            ...
            if isinstance(parameters, dict):
                for k in parameters:
                    if k in dst:
                        ...
                        if isinstance(current_value, tk.array):
                            if strict and not isinstance(new_value, tk.array):
                                raise ValueError(
    ...
    def apply(
        self,
        map_fn: Callable[[tk.array], tk.array],
        ...
        For instance running ``model.apply(lambda x: x.astype(tk.float16))``
    ...
    def modules(self):
        ...
        Returns:
            A list of :class:`tiki.nn.Module` instances.
        """
        modulelist = []
    ...
    def named_modules(self):
        ...
        Returns:
            A list of tuples (str, :class:`tiki.nn.Module`).
        """
        modulelist = []
    ...
    def set_dtype(
        self,
        dtype: tk.Dtype,
        predicate: Optional[Callable[[tk.Dtype], bool]] = lambda x: tk.issubdtype(
            x, tk.floating
        ),
    ):

...
def _sliding_windows(x, window_shape, window_strides):
    ...
    return tk.as_strided(x, final_shape, final_strides)
...
class MaxPool1d(_Pool1d):
    ...
    Examples:
        >>> import tiki as tk
        >>> import tiki.nn.layers as nn
        >>> x = tk.random.normal(shape=(4, 16, 5))
    ...
    def __init__(
        ...
        super().__init__(tk.max, -float("inf"), kernel_size, stride, padding)
...
class AvgPool1d(_Pool1d):
    ...
    def __init__(
        ...
        super().__init__(tk.mean, 0, kernel_size, stride, padding)
...
class MaxPool2d(_Pool2d):
    ...
    * a ``tuple`` of two ``int`` s -- in which case, the first ``int`` is
    ...
    Examples:
        >>> import tiki as tk
        >>> import tiki.nn.layers as nn
        >>> x = tk.random.normal(shape=(8, 32, 32, 4))
...
class MaxPool3d(_Pool3d):
    ...
    * a ``tuple`` of three ``int`` s -- in which case, the first ``int`` is used
    ...
    Examples:
        >>> import tiki as tk
        >>> import tiki.nn.layers as nn
        >>> x = tk.random.normal(shape=(8, 16, 32, 32, 4))

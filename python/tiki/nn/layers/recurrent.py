...
import tiki as tk
from tiki.nn.layers.activations import tanh
...
class RNN(Module):
    ...
    * ``D`` is the input's feature dimension
    ...
    def __init__(
        ...
        self.Wxh = tk.random.uniform(
            low=-scale, high=scale, shape=(hidden_size, input_size)
        )
        self.Whh = tk.random.uniform(
            low=-scale, high=scale, shape=(hidden_size, hidden_size)
        )
        self.bias = (
            tk.random.uniform(low=-scale, high=scale, shape=(hidden_size,))
            if bias
            else None
        )
    ...
    def __call__(self, x, hidden=None):
        if self.bias is not None:
            x = tk.addmm(self.bias, x, self.Wxh.T)
        else:
            x = x @ self.Wxh.T
        ...
        for idx in range(x.shape[-2]):
            if hidden is not None:
                hidden = tk.addmm(x[..., idx, :], hidden, self.Whh.T)
            else:
                hidden = x[..., idx, :]
            hidden = self.nonlinearity(hidden)
            all_hidden.append(hidden)

        return tk.stack(all_hidden, axis=-2)
...
class GRU(Module):
    ...
    * ``D`` is the input's feature dimension
    ...
    def __init__(
        ...
        self.hidden_size = hidden_size
        scale = 1.0 / math.sqrt(hidden_size)
        self.Wx = tk.random.uniform(
            low=-scale, high=scale, shape=(3 * hidden_size, input_size)
        )
        self.Wh = tk.random.uniform(
            low=-scale, high=scale, shape=(3 * hidden_size, hidden_size)
        )
        self.b = (
            tk.random.uniform(low=-scale, high=scale, shape=(3 * hidden_size,))
            if bias
            else None
        )
        self.bhn = (
...
class LSTM(Module):
    ...
    * ``D`` is the input's feature dimension
    ...
    def __init__(
        ...
        self.bias = (
            tk.random.uniform(low=-scale, high=scale, shape=(4 * hidden_size,))
            if bias
            else None
        )
    ...
    def __call__(self, x, hidden=None, cell=None):
        if self.bias is not None:
            x = tk.addmm(self.bias, x, self.Wx.T)
        else:
            x = x @ self.Wx.T
        ...
        for idx in range(x.shape[-2]):
            ifgo = x[..., idx, :]
            if hidden is not None:
                ifgo = tk.addmm(ifgo, hidden, self.Wh.T)
            i, f, g, o = tk.split(ifgo, 4, axis=-1)
            ...
            if cell is not None:
                ...
                cell = i * g
            hidden = o * tk.tanh(cell)

            all_cell.append(cell)
            all_hidden.append(hidden)

        return tk.stack(all_hidden, axis=-2), tk.stack(all_cell, axis=-2)

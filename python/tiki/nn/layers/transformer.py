...
import tiki as tk
from tiki.nn.layers.activations import relu
from tiki.nn.layers.base import Module
from tiki.nn.layers.dropout import Dropout
from tiki.nn.layers.linear import Linear
from tiki.nn.layers.normalization import LayerNorm
from tiki.nn.utils import checkpoint
...
class MultiHeadAttention(Module):
    ...
    def __call__(self, queries, keys, values, mask=None):
        ...
        queries = tk.unflatten(queries, -1, (num_heads, -1)).transpose(0, 2, 1, 3)
        keys = tk.unflatten(keys, -1, (num_heads, -1)).transpose(0, 2, 1, 3)
        values = tk.unflatten(values, -1, (num_heads, -1)).transpose(0, 2, 1, 3)
        scale = math.sqrt(1 / queries.shape[-1])
        output = tk.fast.scaled_dot_product_attention(
    ...
    @staticmethod
    def create_additive_causal_mask(N: int, dtype: tk.Dtype = tk.float32):
        indices = tk.arange(N)
        mask = indices[:, None] < indices[None]
        mask = mask.astype(dtype) * tk.finfo(dtype).min
        return mask
...
class Transformer(Module):
    ...
    Args:
        ...
        activation (function, optional): the activation function for the MLP
            hidden layer. Default: :func:`tiki.nn.relu`.

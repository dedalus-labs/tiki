...
class InstanceNorm(Module):
    ...
    def __init__(
        ...
        if affine:
            self.weight = tk.ones((dims,))
            self.bias = tk.zeros((dims,))
        self.dims = dims
        self.eps = eps
    ...
    def __call__(self, x: tk.array) -> tk.array:
        ...
        channels_first = tk.transpose(x, (0, x.ndim - 1, *range(1, x.ndim - 1)))
        x = tk.fast.layer_norm(
...
class LayerNorm(Module):
    ...
    def __call__(self, x):
        ...
        return tk.fast.layer_norm(x, weight, bias, self.eps)
...
class RMSNorm(Module):
    ...
    def __call__(self, x):
        return tk.fast.rms_norm(x, self["weight"], self.eps)
...
class GroupNorm(Module):
    ...
    def _pytorch_compatible_group_norm(self, x):
        ...
        # Normalize
        x = tk.fast.layer_norm(x, eps=self.eps, weight=None, bias=None)
    ...
    def _group_norm(self, x):
        ...
        x = (x - means) * tk.rsqrt(var + self.eps)
...
class BatchNorm(Module):
    ...
    def __init__(
        ...
        if affine:
            self.weight = tk.ones((num_features,))
            self.bias = tk.zeros((num_features,))

        if self.track_running_stats:
            self.running_mean = tk.zeros((num_features,))
            self.running_var = tk.ones((num_features,))
    ...
    def _calc_stats(self, x: tk.array, ddof: int = 0) -> Tuple[tk.array, tk.array]:
        """
        ...
        mean = tk.mean(x, axis=reduction_axes)
        var = tk.var(x, axis=reduction_axes, ddof=ddof)
    ...
    def __call__(self, x: tk.array) -> tk.array:
        ...
        x = (x - mean) * tk.rsqrt(var + self.eps)
        return (self.weight * x + self.bias) if "weight" in self else x

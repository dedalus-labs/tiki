...
def _reduce(loss: tk.array, reduction: Reduction = "none"):
...
def cross_entropy(
    logits: tk.array,
    targets: tk.array,
    weights: Optional[tk.array] = None,
    axis: int = -1,
    ...
    Examples:
        ...
        >>> logits = tk.array([[2.0, -1.0], [-1.0, 2.0]])
        >>> targets = tk.array([[0.9, 0.1], [0.1, 0.9]])
        >>> nn.losses.cross_entropy(logits, targets)
        ...
        >>> logits = tk.array([[2.0, -1.0], [-1.0, 2.0]], tk.bfloat16)
        >>> targets = tk.array([0, 1])
        >>> nn.losses.cross_entropy(logits, targets)
        array([0.0485873, 0.0485873], dtype=float32)
        >>>
        ...
        >>> nn.losses.cross_entropy(logits.astype(tk.float32), targets)
        array([0.0485873, 0.0485873], dtype=float32)
    """
    ...
    use_fast = (
        tk.cuda.is_available()
        and tk.default_device() == tk.gpu
        and not targets_as_probs
        and label_smoothing == 0
        and axis in (-1, logits.ndim - 1)
        and tk.issubdtype(logits.dtype, tk.floating)
        and tk.issubdtype(targets.dtype, tk.integer)
    )

    if use_fast:
        loss = tk.fast.cross_entropy(logits, targets).astype(logits.dtype)
    else:
        logits = logits - tk.stop_gradient(tk.max(logits, axis=axis, keepdims=True))

        if targets_as_probs:
            score = tk.sum(logits * targets, axis=axis)
        else:
            score = tk.take_along_axis(
                logits, tk.expand_dims(targets, axis), axis
            ).squeeze(axis)

        logsumexp_logits = tk.logsumexp(logits, axis=axis)
...
def binary_cross_entropy(
    ...
    Examples:
        ...
        >>> probs = tk.array([0.1, 0.1, 0.4, 0.4])
        >>> targets = tk.array([0, 0, 1, 1])
    ...
    if with_logits:
        loss = tk.logaddexp(0.0, inputs) - inputs * targets
    else:
        log_inputs_clip = tk.clip(tk.log(inputs), a_min=-100, a_max=None)
        log_inputs_inv_clip = tk.clip(tk.log(1 - inputs), a_min=-100, a_max=None)
...
def l1_loss(
    predictions: tk.array, targets: tk.array, reduction: Reduction = "mean"
) -> tk.array:
    """
    ...
    loss = tk.abs(predictions - targets)
...
def mse_loss(
    ...
    loss = tk.square(predictions - targets)
    return _reduce(loss, reduction)


def nll_loss(
    inputs: tk.array, targets: tk.array, axis: int = -1, reduction: Reduction = "none"
) -> tk.array:
    """
    ...
    """
    loss = -tk.take_along_axis(inputs, tk.expand_dims(targets, axis), axis).squeeze(
        axis
    )
...
def gaussian_nll_loss(
    ...
    # For stability
    vars = tk.maximum(vars, eps)
    loss = 0.5 * (tk.log(vars) + tk.square(targets - inputs) / vars)
...
def kl_div_loss(
    ...
    .. code-block:: python
        ...
        (tk.exp(targets) * (targets - inputs)).sum(axis)
    ...
    """
    loss = tk.sum(tk.exp(targets) * (targets - inputs), axis)

    return _reduce(loss, reduction)


def smooth_l1_loss(
    predictions: tk.array,
    targets: tk.array,
    beta: float = 1.0,
    reduction: Reduction = "mean",
) -> tk.array:
    ...
    diff = tk.abs(predictions - targets)
    loss = tk.where(
        diff < beta, 0.5 * tk.square(diff) / beta, tk.abs(diff) - 0.5 * beta
    )
...
def triplet_loss(
    ...
    """
    pos_dist = tk.power(
        tk.power(tk.abs(anchors - positives), p).sum(axis) + eps, 1.0 / p
    )
    neg_dist = tk.power(
        tk.power(tk.abs(anchors - negatives), p).sum(axis) + eps, 1.0 / p
    )
    loss = tk.maximum(pos_dist - neg_dist + margin, 0)
    return _reduce(loss, reduction)


def hinge_loss(
    inputs: tk.array, targets: tk.array, reduction: Reduction = "none"
) -> tk.array:
    r"""
    ...
    """
    loss = tk.maximum(1 - inputs * targets, 0)
...
def huber_loss(
    ...
    abs_errors = tk.abs(errors)
    quadratic = tk.minimum(abs_errors, delta)
...
def log_cosh_loss(
    ...
    """
    errors = inputs - targets
    loss = tk.logaddexp(errors, -errors) - math.log(2)
...
def cosine_similarity_loss(
    ...
    """
    x1_norm = tk.linalg.norm(x1, axis=axis)
    x2_norm = tk.linalg.norm(x2, axis=axis)

    loss = tk.sum(x1 * x2, axis=axis) / tk.maximum(x1_norm * x2_norm, eps)
...
def margin_ranking_loss(
    ...
    loss = tk.maximum(0, -targets * differences + margin)

...
def constant(
    value: float, dtype: tk.Dtype = tk.float32
) -> Callable[[tk.array], tk.array]:
    ...
    Example:
        ...
        >>> init_fn(tk.zeros((2, 2)))
        array([[0.5, 0.5],
               [0.5, 0.5]], dtype=float32)
    """

    def initializer(a: tk.array) -> tk.array:
        return tk.full(a.shape, value, dtype=dtype)

    return initializer


def normal(
    mean: float = 0.0, std: float = 1.0, dtype: tk.Dtype = tk.float32
) -> Callable[[tk.array], tk.array]:
    ...
    def initializer(a: tk.array) -> tk.array:
        return tk.random.normal(shape=a.shape, scale=std, loc=mean, dtype=dtype)

    return initializer


def uniform(
    low: float = 0.0, high: float = 1.0, dtype: tk.Dtype = tk.float32
) -> Callable[[tk.array], tk.array]:
    ...
    def initializer(a: tk.array) -> tk.array:
        return tk.random.uniform(low, high, a.shape, dtype=dtype)

    return initializer


def identity(dtype: tk.Dtype = tk.float32) -> Callable[[tk.array], tk.array]:
    ...
    def initializer(arr: tk.array) -> tk.array:
        if arr.ndim != 2 or arr.shape[0] != arr.shape[1]:
            raise ValueError(
                f"The input array must be a square matrix but got shape {arr.shape}."
            )
        return tk.eye(n=arr.shape[0], dtype=dtype)
...
def glorot_normal(
    dtype: tk.Dtype = tk.float32,
) -> Callable[[tk.array, float], tk.array]:
    ...
    Example:
        ...
        >>> init_fn(tk.zeros((2, 2)), gain=4.0)
        array([[1.89613, -4.53947],
               [4.48095, 0.995016]], dtype=float32)
    """

    def initializer(a: tk.array, gain: float = 1.0) -> tk.array:
        fan_in, fan_out = _calculate_fan_in_fan_out(a)
        std = gain * math.sqrt(2.0 / (fan_in + fan_out))
        return tk.random.normal(shape=a.shape, scale=std, dtype=dtype)
...
def glorot_uniform(
    ...
    def initializer(a: tk.array, gain: float = 1.0) -> tk.array:
        ...
        return tk.random.uniform(-limit, limit, a.shape, dtype=dtype)
...
def he_normal(
    dtype: tk.Dtype = tk.float32,
) -> Callable[[tk.array, Literal["fan_in", "fan_out"], float], tk.array]:
    ...
    Example:
        ...
        >>> init_fn(tk.zeros((2, 2)))  # uses fan_in
        array([[-1.25211, 0.458835],
               [-0.177208, -0.0137595]], dtype=float32)
        >>> init_fn(tk.zeros((2, 2)), mode="fan_out", gain=5)
...
def sparse(
    ...
    def initializer(a: tk.array) -> tk.array:
        ...
        # every row.
        order = tk.argsort(tk.random.uniform(shape=a.shape), axis=1)
        a = tk.random.normal(shape=a.shape, scale=std, loc=mean, dtype=dtype)

        a[tk.arange(rows).reshape(rows, 1), order[:, :num_zeros]] = 0
...
def orthogonal(
    gain: float = 1.0, dtype: tk.Dtype = tk.float32
) -> Callable[[tk.array], tk.array]:
    ...
    def initializer(a: tk.array) -> tk.array:
        ...
        rmat = tk.random.normal(shape=(n, n))

        # Perform QR decomposition on CPU
        q, r = tk.linalg.qr(rmat, stream=tk.cpu)

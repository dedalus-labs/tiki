...
def value_and_grad(model: Module, fn: Callable):
    ...
    Args:
        model (tiki.nn.Module): The model whose trainable parameters to compute
                               gradients for
    ...
    value_grad_fn = tk.value_and_grad(inner_fn)
...
def checkpoint(module: Module, fn: Optional[Callable] = None):
    ...
    Args:
        module (tiki.nn.Module): The module for whose parameters we will be
    ...
    checkpointed_fn = tk.checkpoint(inner_fn)
...
def average_gradients(
    ...
    communication_stream: Optional[tk.Stream] = None,
):
    ...
    Args:
        ...
        group (Optional[tiki.distributed.Group]): The group of processes to
        ...
        communication_stream (Optional[tiki.Stream]): The stream to use
    ...
    if all_reduce_size <= 0:
        return tree_map(
            lambda x: tk.distributed.all_sum(
                x,
                group=group,
...
def clip_grad_norm_sharded(
    ...
    :func:`tiki.optimizers.clip_grad_norm`. Each member of the group holds only
    ...
    clipping the gradients of a module wrapped with :func:`tiki.nn.fully_shard`.
    ...
    Args:
        ...
        group (Optional[tiki.distributed.Group]): The group across which
            the gradients are sharded. If set to ``None`` the global group is
            used. Default: ``None``.

    Returns:
        (Any, tiki.array): The possibly rescaled local shard of the
        gradients and the global gradient norm.
    """
    local_norm_squared = tree_reduce(
        lambda acc, g: acc + g.square().sum(), gradients, 0.0
    )
    global_norm_squared = tk.distributed.all_sum(local_norm_squared, group=group)
    grad_norm = tk.sqrt(global_norm_squared)
    normalizer = tk.minimum(max_norm / (grad_norm + 1e-6), 1.0)

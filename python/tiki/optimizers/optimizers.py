...
import tiki as tk
from tiki.nn import Module
from tiki.utils import tree_flatten, tree_map, tree_merge, tree_reduce, tree_unflatten
...
class Optimizer:
    ...
    def __init__(self, schedulers=None):
        ...
        self._state = {"step": tk.array(0, tk.uint64)}
    ...
    def update(self, model: Module, gradients: dict):
        ...
        Args:
            model (tiki.nn.Module): An tiki module to be updated.
            gradients (dict): A Python tree of gradients, most likely computed
                              via :func:`tiki.nn.value_and_grad`.
        """
    ...
    def init_single(self, parameter: tk.array, state: dict):
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
    ...
    def learning_rate(self, learning_rate: Union[float, tk.array]):
        self.state["learning_rate"] = tk.array(learning_rate)

    def _maybe_schedule(
        self, name: str, param: Union[float, Callable[[tk.array], tk.array]]
    ):
        """
...
class SGD(Optimizer):
    ...
    def __init__(
        self,
        learning_rate: Union[float, Callable[[tk.array], tk.array]],
        momentum: float = 0.0,
    ...
    def init_single(self, parameter: tk.array, state: dict):
        ...
        state["v"] = tk.zeros_like(parameter)
...
class RMSprop(Optimizer):
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
        ...
        v = state["v"]
        v = alpha * v + (1 - alpha) * tk.square(gradient)
        state["v"] = v

        return parameter - lr * gradient / (tk.sqrt(v) + eps)
...
class AdaDelta(Optimizer):
    ...
    def init_single(self, parameter: tk.array, state: dict):
        """Initialize optimizer state"""
        state["v"] = tk.zeros_like(parameter)
        state["u"] = tk.zeros_like(parameter)
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
        ...
        v = rho * v + (1 - rho) * tk.square(gradient)
        d = tk.sqrt(u + eps) / tk.sqrt(v + eps) * gradient
        u = rho * u + (1 - rho) * tk.square(d)
...
class Adam(Optimizer):
    ...
    def init_single(self, parameter: tk.array, state: dict):
        """Initialize optimizer state"""
        state["m"] = tk.zeros_like(parameter)
        state["v"] = tk.zeros_like(parameter)
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
        ...
        v = b2 * v + (1 - b2) * tk.square(gradient)
        state["m"] = m
        state["v"] = v

        if bias_correction:
            c1 = (lr / (1 - b1**step)).astype(gradient.dtype)
            c2 = tk.rsqrt(1 - b2**step).astype(gradient.dtype)
            numerator = c1 * m
            denominator = tk.sqrt(v) * c2 + eps
            return parameter - numerator / denominator
        else:
            return parameter - lr * m / (tk.sqrt(v) + eps)
...
class Adamax(Adam):
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
        ...
        m = b1 * m + (1 - b1) * gradient
        v = tk.maximum(b2 * v, tk.abs(gradient))
        state["m"] = m
        state["v"] = v
...
class Adafactor(Optimizer):
    ...
    def __init__(
        self,
        learning_rate: Union[float, Callable[[tk.array], tk.array], None] = None,
    ...
    def init_single(self, parameter: tk.array, state: dict):
        ...
        if parameter.ndim >= 2:
            ...
            state["exp_avg_sq_row"] = tk.zeros(shape[:-1], dtype=dtype)
            state["exp_avg_sq_col"] = tk.zeros(shape[:-2] + shape[-1:], dtype=dtype)
        else:
            state["exp_avg_sq"] = tk.zeros_like(parameter)

        if self.beta_1 is not None:
            state["exp_avg"] = tk.zeros_like(parameter)

    def _compute_rms(self, inputs):
        return tk.sqrt(tk.mean(tk.square(inputs)))

    def _compute_learning_rate(self, step, parameter_rms):
        if self.relative_step:
            min_step = 1e-6 * step if self.warmup_init else 1e-2
            relative_step_size = tk.minimum(min_step, tk.rsqrt(step))
        else:
        ...
        if self.scale_parameter:
            parameter_scale = tk.maximum(self.eps[1], parameter_rms)
        return parameter_scale * relative_step_size

    def _approximate_exp_moving_avg(self, exp_avg_sq_row, exp_avg_sq_col):
        r_factor = tk.rsqrt(
            exp_avg_sq_row / tk.mean(exp_avg_sq_row, axis=-1, keepdims=True)
        )
        c_factor = tk.rsqrt(exp_avg_sq_col)
        return tk.expand_dims(r_factor, axis=-1) * tk.expand_dims(c_factor, axis=-2)
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
        ...
        update = tk.square(gradient) + self.eps[0]

        if factored:
            exp_avg_sq_row = state["exp_avg_sq_row"]
            exp_avg_sq_col = state["exp_avg_sq_col"]
            exp_avg_sq_row = (beta_2 * exp_avg_sq_row) + (
                (1 - beta_2) * tk.mean(update, axis=-1)
            )
            exp_avg_sq_col = (beta_2 * exp_avg_sq_col) + (
                (1 - beta_2) * tk.mean(update, axis=-2)
            )
            ...
            update = tk.rsqrt(exp_avg_sq) * gradient

        update = update / tk.maximum(
...
class Muon(Optimizer):
    ...
    def _zeropower_via_newtonschulz5(self, X, steps: int):
        ...
        X = X / (tk.linalg.norm(X, keepdims=True) + 1e-7)

        for _ in range(steps):
            A = X @ X.T
            B = tk.addmm(b * A, A, A, beta=1.0, alpha=c)
            X = tk.addmm(a * X, B, X, beta=1.0, alpha=1.0)
    ...
    def apply_single(self, gradient: tk.array, parameter: tk.array, state: dict):
        ...
        if update.ndim >= 2:
            ...
            if reshape_needed:
                update = tk.reshape(update, (update.shape[0], -1))

            update = self._zeropower_via_newtonschulz5(update, steps=self.ns_steps)

            if reshape_needed:
                update = tk.reshape(update, original_shape)
...
def clip_grad_norm(grads, max_norm):
    ...
    Example:
        >>> grads = {"w1": tk.array([2, 3]), "w2": tk.array([1])}
        >>> clipped_grads, total_norm = clip_grad_norm(grads, max_norm=2.0)
        >>> print(clipped_grads)
        {"w1": tk.array([...]), "w2": tk.array([...])}
    ...
    total_norm = tk.sqrt(norm_squared)
    normalizer = tk.minimum(max_norm / (total_norm + 1e-6), 1.0)

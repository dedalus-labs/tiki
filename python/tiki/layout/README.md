...
Layouts describe coordinates. `ArrayEngine` retains an Tiki array and an integer
element offset. Scalar access uses Tiki indexing and `.item()`, so it can
...
Tiki axis at this boundary. A manually constructed Engine must have a unit-stride
...
assert layout == tl.compose(transform, base)

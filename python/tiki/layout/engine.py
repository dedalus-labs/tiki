...
"""Tiki arrays as Engines, the storage half of a Tiki tensor.

An ``ArrayEngine`` is a flat Tiki buffer plus an element offset. Element reads
and writes go through Tiki indexing, which is exact but slow; that is what the
reference algebra expects of an Accessor. A whole layout is realized as one
zero-copy view by ``tiki.layout.tensor.realize``.
...
import tiki as tk
from tiki.layout._pycute import MutableAccessor
from tiki.layout.layout import LayoutError

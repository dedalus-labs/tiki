...
"""Tiki tensors: an Tiki array Engine composed with a CuTe Layout.
...
import tiki as tk
from tiki.layout._pycute import Shape, Stride, Tensor, flatten, size
...
def realize(tensor: Tensor) -> tk.array:
    ...
    if len(shape) != len(strides) or any(
        ...
        raise LayoutError(
            f"realize needs matching nonnegative Tiki extents and strides, got {shape}"
        )
